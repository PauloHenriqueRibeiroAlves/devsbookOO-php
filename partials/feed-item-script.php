<script> //ARQUIVO 2
    window.addEventListener('load', function() {        
        ...
        //função responsavel por dar o like nas postagens
        ...
        //função responsavel por dar o comentario nas postagens
        document.querySelectorAll('.fic-item-field').forEach(item=>{
            item.addEventListener('keyup', async (e)=>{
                if(e.keyCode == 13) {//numero correspondente pela tecla enter = 13
                    let id = item.closest('.feed-item').getAttribute('data-id');
                    let txt = item.value;
                    item.value = '';
